...
class Args(app.ArgParser):
    def __init__(self):
        ...
        self.parser.add_argument('--output_channel', type=int, default=250)
        self.parser.add_argument('--hidden_size', type=int, default=200)
        self.parser.add_argument('--attn_hidden', type=int, default=200)
        ...
        self.parser.add_argument('--small_batch_size', type=int, default=300)
...
args.batch_size = 200
args.train_txt = 'train{}.comb'.format(args.train_dataset)
args.valid_txt = 'valid{}.comb'.format(args.train_dataset)
args.test_txt = 'test{}.comb'.format(args.train_dataset)
print(args)
...
for fname in ["train{}".format(args.train_dataset),
    ...
    fboundary = open("data/{}.boundary".format(fname))
    boundary = {}
...
def evaluator(name, pairs):
    ...
    if name == "test":
        MAP, MRR, P_30 = evaluation.TWITTER_MAP_MRR(qa_eval_list, pred_fname="pred.test.{}".format(os.getpid()),
                                                    ...
                                                    id_fname="data/test{}.id".format(args.train_dataset),
    ...
    if name == "valid":
        MAP, MRR, P_30 = evaluation.TWITTER_MAP_MRR(qa_eval_list, pred_fname="pred.valid.{}".format(os.getpid()),
                                                    gold_fname="data/qrels.txt",
                                                    id_fname="data/valid{}.id".format(args.train_dataset),
                                                    qid_index=0, docid_index=1, delimiter=' ', model="NN")
        return (n_correct / n_total, P_30)
    if name == "train":
...
def log_printer(name, metrics, loss, epoch=None, iters=None):
    if name == 'train':
        ...
        print("{}\tACC:{}\tP30:{}\tLOSS:{}".format(name, metrics[0], metrics[1], loss))
